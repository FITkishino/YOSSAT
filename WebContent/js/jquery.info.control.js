...
$(function(){
	$(".jump").on('click',function(event){
		...
		var autoQuery="";
		if($('#autoQuery:checked').val()){
			autoQuery="";
		}else{
			autoQuery="&aq=1";
		}
		location.href = $(this).attr('href')+autoQuery;
		return false;
	});
	...
	$.ajax({
		...
		success: function(data){
			...
			//alert(json.rows[0]["SNAPSHOT"][0]["value"]);
			if (json.rows[0]["SNAPSHOT"][0]["value"]==="1"){
				$("#autoQuery").prop('checked',true);
			}else{
				$("#autoQuery").prop('checked',false);
			}
		}
	});

	$("#autoQuery").on('click', function(event){

		// チェック状況取得
		var checked = $('#autoQuery:checked').val();
