...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Report005',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			// 比較対象店舗
			this.setMTenpo(reportno, $.id.SelMTenpo);
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			var szSelMTenpo		= $.getJSONObject(this.jsonString, $.id.SelMTenpo).value;	// 比較対象店舗
			...
			$.post(
				...
				function(json){
					...
					// 削除ボタン状態変化
					$.setButtonState('#'+$.id.btn_delete, true, $.id.btn_delete);
					// 検索ボタン無効化
		...
		delValidation: function (){	// （必須）批准
			...
			// EasyUI のフォームメソッド 'validate' 実施
			var rt = true;
			var seledRows = $($.id.gridholder).datagrid('getChecked');
			if(seledRows.length < 1){
				$.messager.alert($.message.ID_MESSAGE_TITLE_WARN,'削除する店舗を1件以上選択してください。','warning');
				rt = false;
			}
			return rt;
		},
		delSuccess: function(id){
			...
			// Grid内選択情報取得
			var rows = $($.id.gridholder).datagrid('getChecked');
		...
		getEasyUI: function(){	// （必須）情報の取得
			...
			this.jsonTemp.push({
				id:		$.id.SelMTenpo,
				value:	$('#'+$.id.SelMTenpo).combogrid('getValue'),
				text:	$('#'+$.id.SelMTenpo).combogrid('getText')
			});
		},
		...
		setMTenpo: function(reportno, id){		// 比較対象店舗
			var that = this;
			var idx = -1;
			$('#'+id).combogrid({
				panelWidth:250,
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				columns:[[
					{field:'ck',	checkbox:true},
					{field:'F1',	title:'店舗',			width:210,	align:'left',	halign:'center'},
					{field:'F2',	title:'比較対象店舗',	width:210,	align:'left',	halign:'center',
						formatter:function(value, row, index) {
							return '<input type="text" id="F2_'+index+'" style="width: 200px; ime-mode: disabled;" class="TextDisp" tabindex="'+(200+index)+'" value="'+value+'">';
						},
						styler:function(value, row, index) {
							return {class:'yellow'}
						}
					},
					{field:'F3',	title:'開店年月',	width:72,	align:'center',	halign:'center'}
				]],
				...
				onLoadSuccess:function(data){
					...
					var data = $('#'+$.id.SelMTenpo).combogrid('grid').datagrid("getData");
					input.each(function(){
						var initObj = false;
						...
						$(this).combogrid({
							...
							onChange:function(newValue, oldValue){
								if(!initObj) return false;
								$.setChangeIdx(index);
							}
						}).combogrid("setValue", $(this).val());
						initObj = true;
					});
				},
