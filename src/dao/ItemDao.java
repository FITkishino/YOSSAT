...
public class ItemDao implements ItemInterface {
	...
	public boolean selectBySQL(String command) {
		...
		try {
			...
			rollback(con);
			e.printStackTrace();
			if(	DefineReport.ID_SQLSTATE_COLUMN_GREATER.equals(e.getSQLState())
			||	DefineReport.ID_SQLSTATE_APPLICATION_HEPE.equals(e.getSQLState())
			||	DefineReport.ID_SQLSTATE_BUFFER_GREATER.equals(e.getSQLState())
			||	DefineReport.ID_SQLSTATE_COLUMN_OVER.equals(e.getSQLState())
			){
				// 横軸（列）が多すぎる場合
	...
	public String getWeathernewsSql() {
		return " when '100' then '晴'"
				+ " when '101' then '晴時々曇'"
				+ " when '102' then '晴一時雨'"
				+ " when '103' then '晴時々雨'"
				+ " when '104' then '晴一時雪'"
				+ " when '105' then '晴時々雪'"
				+ " when '106' then '晴一時雨か雪'"
				+ " when '107' then '晴時々雨か雪'"
				+ " when '108' then '晴一時雨か雷雨'"
				+ " when '110' then '晴後時々曇'"
				+ " when '111' then '晴後曇'"
				+ " when '112' then '晴後一時雨'"
				+ " when '113' then '晴後時々雨'"
				...
				+ " when '117' then '晴後雪'"
				+ " when '118' then '晴後雨か雪'"
				+ " when '119' then '晴後雨か雷雨'"
				+ " when '120' then '晴朝夕一時雨'"
				+ " when '121' then '晴朝のうち一時雨'"
				+ " when '122' then '晴夕方一時雨'"
				+ " when '123' then '晴山沿い雷雨'"
				+ " when '124' then '晴山沿い雪'"
				+ " when '125' then '晴午後は雷雨'"
				+ " when '126' then '晴昼頃から雨'"
				+ " when '127' then '晴夕方から雨'"
				+ " when '128' then '晴夜は雨'"
				+ " when '129' then '晴夜半から雨'"
				+ " when '130' then '朝のうち霧後晴'"
				+ " when '131' then '晴明け方霧'"
				+ " when '132' then '晴朝夕曇'"
				+ " when '140' then '晴時々雨で雷を伴う'"
				+ " when '160' then '晴一時雪か雨'"
				+ " when '170' then '晴時々雪か雨'"
				+ " when '181' then '晴後雪か雨'"
				+ " when '200' then '曇'"
				+ " when '201' then '曇時々晴'"
				+ " when '202' then '曇一時雨'"
				...
				+ " when '204' then '曇一時雪'"
				+ " when '205' then '曇時々雪'"
				+ " when '206' then '曇一時雨か雪'"
				+ " when '207' then '曇時々雨か雪'"
				+ " when '208' then '曇一時雨か雷雨'"
				+ " when '209' then '霧'"
				+ " when '210' then '曇後時々晴'"
				+ " when '211' then '曇後晴'"
				+ " when '212' then '曇後一時雨'"
				...
				+ " when '217' then '曇後雪'"
				+ " when '218' then '曇後雨か雪'"
				+ " when '219' then '曇後雨か雷雨'"
				+ " when '220' then '曇朝夕一時雨'"
				+ " when '221' then '曇朝のうち一時雨'"
				+ " when '222' then '曇夕方一時雨'"
				+ " when '223' then '曇日中時々晴'"
				+ " when '224' then '曇昼頃から雨'"
				+ " when '225' then '曇夕方から雨'"
				+ " when '226' then '曇夜は雨'"
				+ " when '227' then '曇夜半から雨'"
				+ " when '228' then '曇昼頃から雪'"
				+ " when '229' then '曇夕方から雪'"
				+ " when '230' then '曇夜は雪'"
				+ " when '231' then '曇海上海岸は霧か霧雨'"
				+ " when '240' then '曇時々雨で雷を伴う'"
				+ " when '250' then '曇時々雪で雷を伴う'"
				+ " when '260' then '曇一時雪か雨'"
				+ " when '270' then '曇時々雪か雨'"
				+ " when '281' then '曇後雪か雨'"
				+ " when '300' then '雨'"
				+ " when '301' then '雨時々晴'"
				+ " when '302' then '雨時々止む'"
				...
				+ " when '316' then '雨か雪後晴'"
				+ " when '317' then '雨か雪後曇'"
				+ " when '320' then '朝のうち雨後晴'"
				+ " when '321' then '朝のうち雨後曇'"
				+ " when '322' then '雨朝晩一時雪'"
				+ " when '323' then '雨昼頃から晴'"
				+ " when '324' then '雨夕方から晴'"
				+ " when '325' then '雨夜は晴'"
				+ " when '326' then '雨夕方から雪'"
				+ " when '327' then '雨夜は雪'"
				+ " when '328' then '雨一時強く降る'"
				+ " when '329' then '雨一時みぞれ'"
				+ " when '340' then '雪か雨'"
				+ " when '350' then '雨で雷を伴う'"
				+ " when '361' then '雪か雨後晴'"
				+ " when '371' then '雪か雨後曇'"
				+ " when '391' then '391'"
				+ " when '392' then '392'"
				...
				+ " when '413' then '雪後曇'"
				+ " when '414' then '雪後雨'"
				+ " when '420' then '朝のうち雪後晴'"
				+ " when '421' then '朝のうち雪後曇'"
				+ " when '422' then '雪昼頃から雨'"
				+ " when '423' then '雪夕方から雨'"
				+ " when '424' then '雪夜半から雨'"
				+ " when '425' then '雪一時強く降る'"
				+ " when '426' then '雪後みぞれ'"
				+ " when '427' then '雪一時みぞれ'"
				+ " when '450' then '雪で雷を伴う'"
				+ " when '999' then '欠測'";
	}
}
