...
public class EasyToJSONServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if (DefineReport.ID_DEBUG_MODE)
      System.out.println(outtarget + "=" + nameWith);
    ...
    try {
      ...
      if (outaction.equals(DefineReport.ID_PARAM_ACTION_GET)) {
        ...
        request.getSession().getAttribute(Consts.STR_SES_LOGINUSER);
        ...
        // 店舗
        if (outobj.equals(DefineReport.Select.TENPO.getObj()) || outobj.equals(DefineReport.Select.M_TENPO.getObj())) {
          JSONObject obj = (JSONObject) map.get(0);
          String sqlWhere = "";
          ...
          if (DefineReport.ID_PAGE_005.equals(outpage) && StringUtils.isNotEmpty(kbn) && kbn.equals(DefineReport.ValKizonKbn.NEW.getVal())) {
            sqlcommand =
                "select T1.MISECD as VALUE, T1.MISECD || ' ' || rtrim(T1.TENMEI) as TEXT from SATMS.TENPO_MST T1 where (TENHEH >= TO_CHAR(current date, 'yyyyMMdd') or TENHEH = 0) and exists (select 'X' from SATMS.MTNPJIK M20 where M20.NENTUKI = (SELECT MAX(NENTUKI) FROM SATMS.MTNPJIK) and M20.MISECD = T1.MISECD and M20.KIZONTEN_KBN = '1') order by VALUE";
          } else {
            if (StringUtils.isNotEmpty(kbn)) {
              new ItemList();
              String kijunYm = StringUtils.left(new CmnDate().getToday(), 6);
              ArrayList<String> paramYm = new ArrayList<>();
              paramYm.add(kijunYm);
              paramYm.add(kijunYm);
              @SuppressWarnings("static-access")
              JSONArray kikanInfo =
                  ItemList.selectJSONArray("select case when sum(case when NENTUKI = ? then 1 end) > 0 then ? else max(NENTUKI) end as VALUE from SATMS.MTNPJIK", paramYm, Defines.STR_JNDI_DS);
              if (kikanInfo.size() > 0) {
            ...
            if (obj.containsKey("NENTUKI")) {
              sqlWhere = " inner join (select distinct MISECD from SATMS.TTBMYS where NENTUKI = '" + obj.optString("NENTUKI") + "') T2 on T1.MISECD = T2.MISECD" + sqlWhere;
            }
            sqlcommand = DefineReport.ID_SQL_TENPO_ALL + DefineReport.ID_SQL_TENPO + sqlWhere + DefineReport.ID_SQL_TENPO_FOOTER;
          ...
          if (DefineReport.ID_DEBUG_MODE)
            System.out.println(sqlcommand + "[" + paramData.toString() + "]");
        }
        ...
        if (outobj.equals(DefineReport.Select.BUMON.getObj())) {
          ...
          if (StringUtils.isNotEmpty(sqlWhere)) {
            sqlWhere = " inner join (select distinct BUNBMC from SATMS.TTBMYS " + sqlWhere + ") T2 on T1.BUNBMC = T2.BUNBMC";
          }
          sqlcommand = DefineReport.ID_SQL_BUMON + sqlWhere + DefineReport.ID_SQL_BUMON_FOOTER;

          String sqlWhereFOOTER = "";
          sqlWhereFOOTER = sqlWhere + " group by BUNBMG_S , BMGM_S union all ";
          if (!obj.containsKey("REQUIRED")) {
            sqlcommand = DefineReport.ID_SQL_BUMON_HEAD + DefineReport.ID_SQL_BUMON_HEAD3 + sqlWhereFOOTER + sqlcommand;
          }
        }
      ...
      if (DefineReport.ID_DEBUG_MODE)
        System.out.println("EasyToJSONServletの終了(" + outobj.toString() + ")");
  ...
  private String selectJSON(String sqlCommand, ArrayList<String> paramData, String JNDIname) throws Exception {
    ...
    if (DefineReport.ID_DEBUG_MODE)
      System.out.println("selectJSON:" + sqlCommand);
    String jsonString = "";
    ...
    try {
      ...
      // 項目単位の情報格納
      List<JSONObject> lineData = new ArrayList<>();
