...
public class DefineReport {
	...
	public final static boolean ID_DEBUG_MODE = false;
	...
	public enum DataType {
		...
		public static boolean isText(DataType dataType) {
			return SINGLE_LINE_TEXT.equals(dataType)
					|| MULTI_LINE_TEXT.equals(dataType)
					|| ALPHA.equals(dataType);
		}
	}
	...
	public enum InpText {
		...
		/** @return len 桁数 */
		public int getLen() { return digit1 + digit2; }
		/** @return lbl ラベル */
		public String getLbl() { return txt + LBL_SUFFIX; }
	}
	...
	public final static String ID_SQL_KIKAN_WEEK_INIT = "select NVL(MAX(NENDO), - 1) as NENDO, NVL(MIN(COMTOB), - 1) as MINDT, NVL(MAX(COMTOB), - 1) as MAXDT from INAYS.MCALTT where NENSYUU = (select integer(NENSYUU) - 100 from INAYS.MCALTT where COMTOB = ?)";
	// 登録系画面
	...
	/** 共通（INAYS.MTNPTT） */
	public final static String ID_SQL_TENPO = "select T1.MISECD as VALUE, T1.MISECD|| ' ' || rtrim(T1.TENMEI) as TEXT from INAYS.MTNPTT T1";
	...
	public final static String ID_SQL_TENPO_HEAD2= "select VALUE, TEXT from (values " +
		" ("+Values.TENPO_ALL.getVal()+", '"+Values.TENPO_ALL.getTxt()+"'),("+Values.TENPO_NEW.getVal()+", '"+Values.TENPO_NEW.getTxt()+"'),("+Values.TENPO_EX.getVal()+", '"+Values.TENPO_EX.getTxt()+"')" +
		") as X(value, TEXT) union all ";
	...
	public final static String ID_SQL_TENPO_MIN_CD =
		"SELECT MIN(T1.MISECD) FROM INAMS.TENPO_MST T0 "
		+ "INNER JOIN INAYS.MTNPTT T1 ON T0.MISECD = T1.MISECD"
		;
	...
	// SQL：部門
	public final static String ID_SQL_BUMON_HEAD = "select VALUE, TEXT from (values ('"+Values.NONE.getVal()+"', 'すべて')) as X(value, TEXT) union all ";
	public final static String ID_SQL_BUMON_HEAD2 = "select VALUE, TEXT, VALUE2 from (values ('"+Values.NONE.getVal()+"', 'すべて','"+Values.NONE.getVal()+"')) as X(value, TEXT, value2) union all ";
	public final static String ID_SQL_BUMON_FOOTER = " group by T1.BUNBMC order by VALUE";
	/** 共通（INAYS.MCLSTT） */
	public final static String ID_SQL_BUMON = "select T1.BUNBMC as VALUE, T1.BUNBMC || ' ' || rtrim(max(T1.TOUKATU_NM_S)) as TEXT from INAYS.MCLSTT T1";

	// SQL：部門荒利率
	public final static String ID_SQL_BMN_ARA_RIT = "select case when max(BUNARP) = min(BUNARP) then max(BUNARP) end as VALUE from INAMS.TTBMYS where MISECD = ? and NENTUKI = ? ";
	...
	public static final String ID_SQL_KEY_HEAD =
		"select VALUE, TEXT from (values ('', '')) as X(value, TEXT) union all " +
		"select TO_CHAR(CD_CTG) as VALUE, NM_CTG as TEXT from KEYSYS.T_CTG_HEAD where CD_USER = ? order by TEXT";
	...
	/** 商品条件（T_CTG_ITEM）削除	 */
	public static final String ID_SQL_KEY_DELETE_ITEM =
		"delete from KEYSYS.T_CTG_ITEM where CD_CTG in (select CD_CTG from KEYSYS.T_CTG_HEAD where CD_USER = ? and NM_CTG = ?)";
	...
	public static final String ID_SQL_KEY_DELETE_HEAD_SP =
		"delete from KEYSYS.T_CTG_HEAD where CD_CTG = ?";

	/** 商品条件（T_CTG_ITEM）削除専用	 */
	public static final String ID_SQL_KEY_DELETE_ITEM_SP =
		"delete from KEYSYS.T_CTG_ITEM where CD_CTG = ?";

	/** SQL：商品検索（JANコード） */
	public final static String ID_SQL_SYOHIN_JAN =
		"SELECT VALUE, TEXT FROM (SELECT SYOCD AS VALUE, SYOM AS TEXT FROM INATR.SYOUHIN_MST WHERE (SYOCD LIKE ? OR SYOM LIKE ? ) FETCH FIRST 30 ROWS ONLY) ORDER BY TEXT";
	...
	public static final String ID_SQL_KEY_HEAD_TG =
		"select VALUE, TEXT from (values ('', '')) as X(value, TEXT) union all " +
		"select TO_CHAR(CD_CTG) as VALUE, NM_CTG as TEXT from KEYSYS.T_TNP_HEAD where CD_USER = ? order by TEXT";
	...
	public static final String ID_SQL_KEY_DELETE_ITEM_TG =
		"delete from KEYSYS.T_TNP_ITEM where CD_CTG in (select CD_CTG from KEYSYS.T_TNP_HEAD where CD_USER = ? and NM_CTG = ?)";
	...
	public static final String ID_SQL_KEY_DELETE_HEAD_SP_TG =
		"delete from KEYSYS.T_TNP_HEAD where CD_CTG = ?";

	/** 店舗グループ（T_TNP_ITEM）削除専用	 */
	public static final String ID_SQL_KEY_DELETE_ITEM_SP_TG =
		"delete from KEYSYS.T_TNP_ITEM where CD_CTG = ?";
	 ...
	 */
	/** 検索条件の削除 */
	public static final String ID_SQL_DELETE_SHIORI	=
		"DELETE FROM KEYSYS.T_SHIORI WHERE CD_SHIORI=?";
	/** 検索条件の登録 */
	public static final String ID_SQL_INSERT_SHIORI	=
		"INSERT INTO KEYSYS.T_SHIORI(CD_USER,CD_REPORT,NM_SHIORI,SNAPSHOT,FG_PUBLIC) VALUES(?,?,?,?,?)";
	/** SQL：定義検索 */
	...
	public static final String ID_SQL_INSERT_SYSLOGS =
		"insert into KEYSYS.SYS_LOGS(CD_USER,DT_ACTION,CD_ACTION,REMARK,ID_USER,ID_REPORT) values(?,current_timestamp,?,?,?,?)";
