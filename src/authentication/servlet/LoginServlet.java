...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			// パスワード有効期限確認
			if(!vld.IsPasswordTrim(lUser)) {
				/* エラーメッセージの設定 */
				request.setAttribute(Defines.STR_FRM_MSGTYPE, Defines.STR_VAL_MSGTYPE_ERR);
				request.setAttribute(Defines.STR_FRM_MSG, Defines.MSG_LUSER_PASSOUT);
			...
			// TODO:いなげや特処理
			loginusr.setPos_yos_custom_value_();
...
//			}
				...
				for (int i = 0; i < menu.size(); i++) {
					...
					if (list != null) {
						...
						for (int j = 0; j < list.size(); j++) {
							...
							// レポート分類マスタ
							//list1 = (List) qr.query(SQL.REPORT_PARAMETER_002,new Object[] {new Long(mu.get("CD_REPORT_SIDE").toString().trim()) },rsh);
							// グループマスタ
							//list2 = (List) qr.query(SQL.REPORT_PARAMETER_003,new Object[] {new Long(mu.get("CD_REPORT_NO").toString().trim()),new Long(usr.getCD_user()) },rsh);
							// ロールマスタ
							//list3 = (List) qr.query(SQL.REPORT_PARAMETER_004,new Object[] {new Long(mu.get("CD_REPORT_NO").toString().trim()),new Long(usr.getCD_user()) },rsh);
			...
			//パスワード有効期限の残日数によりメッセージ設定 31日以下ならメッセージ設定
			if (lUser.hasAdmin() == false) {
				CmnDate cdate = new CmnDate();
				String chkdate = cdate.addDate_String(null, 0, 0, 31);

				if (lUser.getPwdate(Defines.DT_PW_FORMAT_SAVE).toString().compareTo(chkdate) <= 0) {
					//パスワード有効期限警告メッセージ作成
					String msg_pw_warning = "パスワードの有効期限が";
					msg_pw_warning += lUser.getPwdate(Defines.DT_PW_FORMAT_DISP).substring(0,4) + "年" + lUser.getPwdate(Defines.DT_PW_FORMAT_DISP).substring(4,6) + "月" + lUser.getPwdate(Defines.DT_PW_FORMAT_DISP).substring(6,8) + "日";
					msg_pw_warning += "で切れます。ホスト側のパスワードを変更してください。"
						+ "\\nパスワードは8桁以上で登録してください。"
						+ "\\nなお、ATLASへ変更が反映されるのは変更後の翌日からとなりますのでご注意ください。";
					//メッセージ設定
					request.setAttribute(Defines.STR_SES_PASS_TERM_MSG, msg_pw_warning);
				}
			}
